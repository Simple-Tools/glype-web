...
class parser {
	...
	******************************************************************/
	...
	function HTMLDocument($input, $insert='', $inject=false, $footer='') {
		...
		global $charset;
		if (!isset($charset)) {
			$meta_equiv = preg_match('#(<meta[^>]*http\-equiv\s*=[^>]*charset[^>]*>)#is', $input, $tmp, PREG_OFFSET_CAPTURE) ? $tmp[0][0] : null;
...
echo($meta_equiv);
		...
		//var_dump($charset);
		if(!isset($charset)){
			$input = mb_convert_encoding($input, 'HTML-ENTITIES', "GB2312");
	...
	function JS($input) {
		...
		while ( $offset < $length ) {
			...
			foreach ( $searchStrings as $item ) {
				...
				if ( $tmp-1>0 && ( ( $input[$tmp-1] == '_' ) || ctype_alpha($input[$tmp-1]) )) {
